...
class CustomRetrievalMetric(Metric, ABC):
    ...
    def update(self, preds: torch.Tensor, candidate_aspects: torch.Tensor, clicked_aspects: torch.Tensor, cand_indexes: torch.Tensor, hist_indexes: torch.Tensor) -> None:
        ...
        cand_indexes, preds, candidate_aspects = _check_retrieval_inputs(
            cand_indexes, preds, candidate_aspects, allow_non_binary_target=self.allow_non_binary_target, ignore_index=self.ignore_index
        )
    ...
    @abstractmethod
    def _metric(self, preds: torch.Tensor, candidate_aspects: torch.Tensor, clicked_aspects: torch.Tensor) -> torch.Tensor:
